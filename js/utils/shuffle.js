...
class Cell {
  ...
  constructor(DOM_el, { position, previousCellPosition } = {}) {
    ...
    this.bkgColor = this.originalBkgColor = getComputedStyle(
      document.documentElement
...
const randomlyPicked = options[Math.floor(randomNumber(0, options.length - 1))];
/**
...
export default class TypeShuffle {
  ...
  fx1() {
    ...
    const loop = (line, cell, iteration = 0) => {
      ...
      if (iteration === MAX_CELL_ITERATIONS - 1) {
        ...
        cell.color = ["	#D9DDDC", "#808588", "#363636"][
    ...
    for (const line of this.lines) {
      for (const cell of line.cells) {
        setTimeout(
          () => loop(line, cell),
          (line.position + 1) * 100 * randomNumber(1, 4)
        );
      }
    }
  }
  ...
  reader() {
    ...
    for (const line of this.lines) {
      for (const cell of line.cells) {
        setTimeout(
          () => loop(line, cell),
          (cell.position + 1) * 10 * randomNumber(1, 4)
        );
      }
    }
  }
  ...
  clearReader() {
    ...
    for (const line of this.lines) {
      for (const cell of line.cells) {
        setTimeout(
          () => loop(line, cell),
          (cell.position + 1) * 5 * randomNumber(1, 4)
        );
      }
    }
  }
  ...
  fx6() {
    ...
    for (const line of this.lines) {
      for (const cell of line.cells) {
        setTimeout(() => loop(line, cell), (line.position + 1) * 1000);
      }
    }
  }
